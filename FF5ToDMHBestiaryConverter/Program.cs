    ...
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            using (StringWriter writer = new StringWriter())
            {
                ...
                string filename = Path.GetDirectoryName(args[0]) + Path.DirectorySeparatorChar + Path.GetFileNameWithoutExtension(args[0]) + "_dmhBestiary.xml";
        ...
        private static void MapValues(FC5Compendium compendium, DmhRoot root)
        {
            ...
            foreach (FC5Monster monster in compendium.Monsters)
            {
                ...
                if (monster.Actions != null)
                {
                    foreach (FC5Action action in monster.Actions)
                    {
                        ...
                        List<DmhAttack> attacks = new List<DmhAttack>();

                        foreach (string attack in action.Attacks)
                        {
                            ...
                            DmhAttack dmhAttack = new DmhAttack();

                            string[] parts = attack.Split(new[] {"|"}, StringSplitOptions.None);
                            dmhAttack.Description = string.IsNullOrWhiteSpace(parts[0]) ? action.Name : parts[0];
                            dmhAttack.AttackBonus = parts[1];
                            string[] boniDice = parts[2].Split(new[] {"+"}, StringSplitOptions.None);
                            dmhAttack.DamageBonus += (boniDice.Length > 1 ? string.Join("+", boniDice.Skip(1)) : "0");
                            dmhAttack.DamageDice += boniDice[0];

                            attacks.Add(dmhAttack);
                        }

                        element.Attacks = attacks.ToArray();
                ...
                if (monster.Legendaries != null)
                {
                    ...
                    {
                        DmhLegendaryActionElement element = new DmhLegendaryActionElement {Name = legendary.Name, Desc = string.Join("\r\n", legendary.Texts)};
                        ...
                        foreach (string attack in legendary.Attacks)
                        {
                            if (string.IsNullOrWhiteSpace(attack))
                            {
                                continue;
                            ...
                            dmhAttack.Description = string.IsNullOrWhiteSpace(parts[0]) ? legendary.Name : parts[0];
                            dmhAttack.AttackBonus = parts[1];
                            string[] boniDice = parts[2].Split(new[] {"+"}, StringSplitOptions.None);
                            dmhAttack.DamageBonus += (boniDice.Length > 1 ? string.Join("+", boniDice.Skip(1)) : "0");
                            dmhAttack.DamageDice += boniDice[0];
                ...
                if (monster.Traits != null)
                {
                    ...
                    {
                        DmhSpecialAbilityElement element = new DmhSpecialAbilityElement {Name = trait.Name, Desc = string.Join("\r\n", trait.Texts)};
                        ...
                        foreach (string attack in trait.Attacks)
                        {
                            if (string.IsNullOrWhiteSpace(attack))
                            {
                                continue;
                            ...
                            string[] parts = attack.Split(new[] {"|"}, StringSplitOptions.None);
                            dmhAttack.Description = string.IsNullOrWhiteSpace(parts[0]) ? trait.Name : parts[0];
                            dmhAttack.AttackBonus = parts[1];
                            string[] boniDice = parts[2].Split(new[] {"+"}, StringSplitOptions.None);
                            dmhAttack.DamageBonus += (boniDice.Length > 1 ? string.Join("+", boniDice.Skip(1)) : "0");
                            dmhAttack.DamageDice += boniDice[0];
                ...
                if (monster.Reaction != null)
                {
                    ...
                    else
                    {
                        ...
                        foreach (string attack in monster.Reaction.Attack)
                        {
                            if (string.IsNullOrWhiteSpace(attack))
                            {
                                continue;
                            ...
                            string[] parts = attack.Split(new[] {"|"}, StringSplitOptions.None);
                            dmhAttack.Description = string.IsNullOrWhiteSpace(parts[0]) ? monster.Reaction.Name : parts[0];
                            dmhAttack.AttackBonus = parts[1];
                            string[] boniDice = parts[2].Split(new[] {"+"}, StringSplitOptions.None);
                            dmhAttack.DamageBonus += (boniDice.Length > 1 ? string.Join("+", boniDice.Skip(1)) : "0");
                            dmhAttack.DamageDice += boniDice[0];
                        ...
                        reactionElement.Attacks = attacks.ToArray();
                ...
                bestiaryElement.ArmorClass = monster.AC.Split(new []{" "},StringSplitOptions.None)[0];
                bestiaryElement.ChallengeRating = monster.CR;
                ...
                bestiaryElement.HitDice = monster.HP.Contains("(") ? monster.HP.Split(new[] {"("}, StringSplitOptions.None)[1].Replace(")", "") : monster.HP;
            ...
            DmhLicense license = new DmhLicense();
            ...
            license.Element = licenseStrings.ToArray();

            root.Bestiary.License = license;
        }
    }
}